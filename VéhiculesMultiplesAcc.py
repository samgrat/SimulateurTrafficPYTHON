...
def accelerate():
    ...
    if i[j][2] + i[j][7]*1/FRAMES_PER_SEC <= SPEED_LIMIT/3.6 : # si le véhicule ne va pas dépasser la limite de vitesse (km/h vers m/s: /3.6)
        i[j][2] += i[j][7]*1/FRAMES_PER_SEC # la boucle s'actualise toute les 1000/FRAMES_PER_SEC ms d'ou le * 1/FRAMES_PER_SEC
    elif i[j][2] > SPEED_LIMIT/3.6:                     # si le véhicule a dépassé la vitesse limite il freine
        i[j][2] -= i[j][7]*1/FRAMES_PER_SEC
...
for k in range(BALLS):
    def majAcc(nouvelleValeur, k=BALLS-k-1):
        ...
        i[k][7] = float(nouvelleValeur)
        print ("Acceleration ", "car ",BALLS-k-1,' ',i[k][7])
    fmajAcc[k] = majAcc
    print (k)
...
for k in range(BALLS): # on crée une liste qui contient tous les véhicules
    i[BALLS-1 - k]= [myCan.create_oval(BALL_RADIUS, HEIGHT/2-BALL_RADIUS, BALL_RADIUS, HEIGHT/2+BALL_RADIUS,fill = Colors[k])
        ,0+k*(BALL_RADIUS*2+BALL_RADIUS/10),StartSPEED,False,False,WIDTH-BALL_RADIUS, 0, ACCELERATION]
